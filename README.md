...
| 特性 | 实现方式 |
|------|---------|
| 消息采集 | pywinauto + pyautogui 自动化操作企业微信窗口 |
| 消息解析 | 正则匹配企微复制格式，兼容有/无年份两种日期格式 |
| 数据存储 | SQLite + Fernet 对称加密，content 列入库即加密 |
| Excel 导出 | openpyxl 生成带专业排版的 .xlsx 文件 |
| 窗口界面 | pywebview 内嵌 React + Ant Design 暗色主题 |
| 代码保护 | PyArmor 加密核心模块，PyInstaller 打包为单目录可执行文件 |
| 授权验证 | RSA 签名 + 机器指纹绑定（CPU ID + 主板 UUID + 磁盘序列号） |
| 防篡改 | 加密时间水位线检测时钟回拨，多重检测虚拟机环境 |
